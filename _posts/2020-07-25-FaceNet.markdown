...
ในบทความนี้จะแสดงการเทรน neural network ด้วย triplet loss แบบง่าย ๆ ให้ดู เพื่อแสดงให้เห็นว่า triplet loss นั้นสามารถทำงานได้จริง ๆ และโค้ดโดยคร่าวนั้นจะเป็นอย่างไร ซึ่งข้อมูลที่ใช้นั้นจะขอใช้เป็นภาพ mnist แค่ triplet เดียว (3 ภาพ) แทน dataset หน้าคนแล้วกัน เพราะมันน่าจะเทรนด้วยได้ง่ายกว่า แล้วก็ใช้ embedding vector ที่มีขนาดแค่ 2 ก็น่าจะพอ ซึ่งจะง่ายต่อการ visualize ให้ดูกัน

> ใน Facenet จริง ๆ นั้นใช้พวก GoogleLeNet, Inception model โดยที่มีขนาด output หรือ embedding vector เป็น 128

1. ก่อนอื่นก็ขอเลือก triplet ออกมาอันนึงก่อน (3 ภาพ) เป็นภาพเลข 1 สองภาพ และภาพเลข 7 หนึ่งภาพ
	```python
	train,test = tf.keras.datasets.mnist.load_data(path="mnist.npz")
	anchor = train[0][train[1]==1][0]
	positive = train[0][train[1]==1][1]
	negative = train[0][train[1]==7][4]
	f, axes = plt.subplots(1,3, facecolor='white', figsize=(15,5))
	axes[0].set_title('Anchor')
	axes[0].imshow(anchor, cmap='gray')
	axes[1].set_title('Positive')
	axes[1].imshow(positive, cmap='gray')
	axes[2].set_title('Negative')
	axes[2].imshow(negative, cmap='gray')
	```
...
2. ต่อมาก็สร้างโมเดล เอาแบบง่าย ๆ เลย กับสร้าง optimizer เตรียมไว้
	...
	dense = Dense(500, activation='relu')(input_layer)
